...
def index(request):
    return HttpResponse("HElolo there hehe")

def information_of_users(request):
    
    return render(request, "countries/allinfo.html", {
...
def countries_details(request, pk):
    ...
    if request.method == 'GET':
        ...
        countries.delete()
        return JsonResponse({'message': 'Country was succesfully deleted.'}, status=status.HTTP_204_NO_CONTENT)
